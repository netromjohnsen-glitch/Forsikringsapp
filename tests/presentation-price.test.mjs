...
test('status plus sum places the sum in details once',()=>{const d=details([fact('rettshjelp.dekning','Valgt'),fact('rettshjelp.sum','100 000 kr')]);assert.equal(d.hasAdditional,true);assert.equal(d.additional.filter(r=>r.key==='rettshjelp.sum').length,1);});
test('rendered formatting does not determine detail availability',()=>{const d=details([fact('rettshjelp.dekning','Valgt'),fact('rettshjelp.sum','100 000 kr')],[fact('rettshjelp.dekning','Ja'),fact('rettshjelp.sum','100000 kroner')]);assert.equal(d.hasAdditional,true);});
...
test('freeform effective summary is preserved without invented subfacts',()=>{const value='Forsikringssum 100 000 kr. Gjelder tvist som eier, bruker eller fører i Norden.';const d=details([fact('rettshjelp.dekning',value)]);assert.ok(d.compact.some(r=>r.firstDescription===value));assert.ok(!d.compact.some(r=>r.key==='rettshjelp.sum'));});
...
test('actual UI hides empty expander and retains source controls',()=>{const html=renderDetails([fact('rettshjelp.dekning','Valgt')]);assert.ok(!html.includes('Se detaljer'));assert.ok(html.includes('Vis kilde'));assert.ok(html.includes('✓ Valgt'));});
test('actual expander uses native disclosure semantics and has additional content',()=>{const html=renderDetails([fact('rettshjelp.dekning','Valgt'),fact('rettshjelp.sum','100000 kr'),fact('rettshjelp.egenandel','4 000 kr + 20 %')]);assert.ok(html.includes('Se detaljer'));assert.match(html,/<details class="group mt-2"><summary/);const expanded=html.split('<details class="group mt-2">')[1];assert.ok(expanded.includes('4 000 kr'));assert.ok(expanded.includes('100000 kr'));assert.ok(html.includes('sm:grid-cols-2'));});
test('sources follow their fact inside expandable details',()=>{const html=renderDetails([fact('rettshjelp.dekning','Valgt'),fact('rettshjelp.geografi','Norden')]);const expanded=html.split('<details class="group mt-2">')[1];assert.ok(expanded.includes('Vis kilde: Eksisterende – rettshjelp.geografi: Norden'));assert.ok(!html.includes('Kilder – eksisterende'));});
